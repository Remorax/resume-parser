...
def BulkParsing(uid):
	...
	for i in range(0,len(myfiles)):
		...
		b = GenericResumeParser(str(testDocument))
		ret = b.AllParsersCall2()
		print (ret)
		csvList.append([myfiles[i],ret])
	return csvList

def BulkParsingTxt(uid):
	import os
	from GenericParserClass2 import GenericResumeParser
	BASE = os.path.dirname(os.path.abspath(__file__))
	path = '/BulkUploadResumes/'
	fullpath =  BASE+path+uid+"/"
	files = os.listdir(fullpath)
	myfiles = [i for i in files if i.endswith('.txt')]
	csvList = []
	for i in range(0,len(myfiles)):
		print(myfiles[i])
		b = GenericResumeParser(str(open("BulkUploadResumes/tester/" + myfiles[i],'r').read()))
		ret = b.AllParsersCall2()
		print (ret)
		csvList.append([myfiles[i],ret])
	return csvList
...
print (getAllResumeDetails("","tester",1))
