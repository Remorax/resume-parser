...
allDetails = {}
allCount = 0
def GetAllDetails(q,o):
	global allDetails
	global allCount
	if q=="edu":
		allDetails["Education"]=o.EducationParser()
		allCount +=1
	if q=="exp":
		allDetails["Experience"]=o.ExperienceParser()
		allCount +=1
	if q=="skill":
		allDetails["Skills"]=o.SkillsParser()
		allCount +=1

def DetailsInThread(testDocument):
	# testDocument = textract.process("../ResumeForPilotCheck/"+i)
	a = GenericResumeParser(testDocument)
	try:
	   thread.start_new_thread( GetAllDetails, ("edu", a, ) )
	   thread.start_new_thread( GetAllDetails, ("exp", a, ) )
	   thread.start_new_thread( GetAllDetails, ("skill", a, ) )
	except:
	...
	# DetailsInThread(testDocument)
	# allCount = 0
	# print allDetails
	# csvList.append([i,allDetails])
	# allDetails = dict()
